  ...
  */
  ...
  void Render() {
    ...
    for (auto& render_item : normal_items) {
      float dist = render_item.model->TraceSortSphere(camera.GetDir(), camera.GetPosition(), render_item.transform);
    ...
    for (auto& BARI : bone_animated_items_to_render) {
      float dist = BARI.model->TraceSortSphere(camera.GetDir(), camera.GetPosition(), BARI.transform);;
      CombinedRenderItem item;
