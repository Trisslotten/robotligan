...
ShaderProgram prepass_model_shader;
ShaderProgram prepass_animated_model_shader;
...
void Init() {
  ...
  prepass_model_shader.add("modelshader.vert");
  prepass_model_shader.add("shading.vert");
  prepass_model_shader.compile();
  prepass_animated_model_shader.add("animatedmodelshader.vert");
  prepass_animated_model_shader.add("shading.vert");
  prepass_animated_model_shader.compile();

  mesh_render_group.push_back(&model_shader);
  mesh_render_group.push_back(&animated_model_shader);
  mesh_render_group.push_back(&prepass_model_shader);
  mesh_render_group.push_back(&prepass_animated_model_shader);
...
void DrawRenderItem(const RenderItem& render_item) {
  model_shader.use();
  ...
  model_shader.uniform("dynamic_em_strength", render_item.emission_strength);
  render_item.model->Draw(model_shader);
}

void DrawAnimatedRenderItem(const BoneAnimatedRenderItem& BARI) {
  ...
  animated_model_shader.uniformv("bone_transform", BARI.bone_transforms.size(),
                                 BARI.bone_transforms.data());
  animated_model_shader.uniform("normal_transform",
                                calcNormalTransform(BARI.transform));
  animated_model_shader.uniform("dynamic_em_strength", BARI.emission_strength);
...
void Render() {
  ...
  // split transparent and opaque object
  std::multimap<float, CombinedRenderItem> combined_items;
  std::multimap<float, CombinedRenderItem> transparent_combined_items;
  for (auto& render_item : items_to_render) {
    ...
    if (render_item.model->IsTransparent()) {
      float max_dist = render_item.model->MaxDistance(render_item.transform,
                                                      camera.GetPosition());
      transparent_combined_items.insert(std::make_pair(-max_dist, item));
    } else {
  ...
  for (auto& BARI : bone_animated_items_to_render) {
    ...
    item.item = &BARI;
    if (BARI.model->IsTransparent()) {
      float max_dist =
          BARI.model->MaxDistance(BARI.transform, camera.GetPosition());
      transparent_combined_items.insert(std::make_pair(-max_dist, item));
    } else {
  ...
  post_process.BeginRender();
  {
    ...
    model_shader.use();
    for (auto& [dist, item] : transparent_combined_items) {
      if (item.is_animated) {
        auto& BARI = *reinterpret_cast<BoneAnimatedRenderItem*>(item.item);
        DrawAnimatedRenderItem(BARI);
      } else {
