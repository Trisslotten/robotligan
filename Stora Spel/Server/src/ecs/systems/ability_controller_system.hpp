...
namespace ability_controller {
  ...
  void Update(entt::registry& registry, float dt) {
    auto view_players =
      registry.view<PlayerComponent, TransformComponent, AbilityComponent, IDComponent>();
    for (auto player : view_players) {
      ...
      if (player_component.invisible && player_component.invisibility_remaining <= 0.0f) {
        ...
        std::cout << "Invisibility END" << std::endl;
      }
  ...
  bool DoInvisibility(entt::registry& registry, PlayerID id)
  {
    ...
    for (auto entity : view_controller) {
      ...
      if (pc.client_id == id) {
        ...
        pc.invisible = true;
        pc.invisibility_remaining = GlobalSettings::Access()->ValueOf("ABILITY_INVISIBILITY_DURATION");
        ...
        std::cout << "Invisibility START" << std::endl;
