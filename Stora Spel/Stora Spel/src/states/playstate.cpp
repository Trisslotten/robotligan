...
void PlayState::MovePlayer(float dt) {
	if (!free_look_) {
		...
		auto& cam_o =
			registry_gameplay_.get<CameraComponent>(my_entity_).orientation;
...
void PlayState::CreateTechDemoScene() {
	...
	light_c1.color = glm::vec4(1.0, .1, .1, 1.0);
	light_c1.radius = 15.f;
	light_c1.ambient = 0;
	registry_gameplay_.assign<TransformComponent>(l_corner_light1, glm::vec3(31, -6, 15));
	l_corner_light2 = registry_gameplay_.create();
	auto& light_c2 = registry_gameplay_.assign<LightComponent>(l_corner_light2);
	light_c2.color = glm::vec4(.1, 1.0, .1, 1.0);
	light_c2.radius = 15.f;
	light_c2.ambient = 0;
	registry_gameplay_.assign<TransformComponent>(l_corner_light2, glm::vec3(33, -4, 15));
	l_corner_light3 = registry_gameplay_.create();
	auto& light_c3 = registry_gameplay_.assign<LightComponent>(l_corner_light3);
	light_c3.color = glm::vec4(.1, .1, 1.0, 1.0);
	light_c3.radius = 15.f;
	light_c3.ambient = 0;
	registry_gameplay_.assign<TransformComponent>(l_corner_light3, glm::vec3(28, -4, 17));

	glob::ModelHandle hndl_cube1 = glob::GetModel("assets/test_cube/cube_1.fbx");
	...
	cube_1_m_c.handles.push_back(hndl_cube1);
	registry_gameplay_.assign<TransformComponent>(cube_1, glm::vec3(42, -9.5, 19));

	auto cube_2 = registry_gameplay_.create();
	auto& cube_2_m_c = registry_gameplay_.assign<ModelComponent>(cube_2);
	cube_2_m_c.handles.push_back(hndl_cube2);
	registry_gameplay_.assign<TransformComponent>(cube_2, glm::vec3(42, -9.5, 22));

	auto cube_3 = registry_gameplay_.create();
	auto& cube_3_m_c = registry_gameplay_.assign<ModelComponent>(cube_3);
	cube_3_m_c.handles.push_back(hndl_cube3);
	registry_gameplay_.assign<TransformComponent>(cube_3, glm::vec3(42, -9.5, 25));

	auto pickup = registry_gameplay_.create();
	auto& pickup_m_c = registry_gameplay_.assign<ModelComponent>(pickup);
	pickup_m_c.handles.push_back(hndl_pickup);
	auto& pickup_trans_c = registry_gameplay_.assign<TransformComponent>(pickup, glm::vec3(35, -9.5, 25));
	pickup_trans_c.scale = glm::vec3(0.3f);

	auto window1 = registry_gameplay_.create();
...
void PlayState::UpdateTechDemo(float dt)
{
	if (Input::IsKeyPressed(GLFW_KEY_B)) {
		glob::CreateBlackHole(glm::vec3(-10, -10, 10));
	}
	...
	auto& l_light_trans_c2 = registry_gameplay_.get<TransformComponent>(l_corner_light2);
	auto& l_light_trans_c3 = registry_gameplay_.get<TransformComponent>(l_corner_light3);
	auto orig_pos1 = glm::vec3(36, -9, 15);
	auto orig_pos2 = glm::vec3(38, -8, 20);
	auto orig_pos3 = glm::vec3(40, -7, 10);
	l_light_trans_c1.position.x = orig_pos1.x + std::sin(demo_anim_timer.Elapsed());
	l_light_trans_c1.position.y = orig_pos1.y + std::sin(demo_anim_timer.Elapsed());
	l_light_trans_c1.position.z = orig_pos1.z + std::sin(demo_anim_timer.Elapsed());
	l_light_trans_c2.position.x = orig_pos2.x - std::sin(demo_anim_timer.Elapsed());
	l_light_trans_c2.position.y = orig_pos2.y + std::sin(demo_anim_timer.Elapsed());
	l_light_trans_c2.position.z = orig_pos2.z + std::sin(demo_anim_timer.Elapsed());
...
void PlayState::CreateArenaEntity() {
	...
	model_c.cast_shadow = true;

	registry_gameplay_.assign<TransformComponent>(arena, zero_vec - glm::vec3(0, 11.5, 0), zero_vec,
		arena_scale);
