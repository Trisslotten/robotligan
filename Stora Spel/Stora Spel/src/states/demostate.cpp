...
void DemoState::Startup() {
    ...
    registry_demo_.on_destroy<ParticleComponent>()
        .connect<&DemoState::ParticleComponentDestroyed>(*this);

    registry_demo_.on_destroy<SoundComponent>()
        .connect<&DemoState::SoundComponentDestroyed>(*this);
}

void DemoState::CreateGoalParticles(float x, entt::registry& registry) {
    ...
    // auto ball_view = registry_demo_.view<
...
void DemoState::Init() {
    registry_demo_.reset();
    glob::window::SetMouseLocked(true);
    engine_->SetSendInput(true);
    engine_->SetCurrentRegistry(&registry_demo_);
    ...
    engine_->GetSoundSystem().PlayAmbientSound(registry_demo_);
...
void DemoState::Update(float dt) {
    ...
    if (!player_look_dirs_.empty()) {
        auto view_entities =
            registry_demo_.view<PlayerComponent, IDComponent>();
    ...
    if (!physics_.empty()) {
        auto view_entities =
            registry_demo_.view<PhysicsComponent, IDComponent>();
        for (auto entity : view_entities) {
    ...
    if (!transforms_.empty()) {
        auto view_entities =
            registry_demo_.view<TransformComponent, IDComponent>();
    ...
    for (auto entity : view_entities) {
        ...
        if (id_c.id == my_id_) {
            ...
            auto& cam_c = registry_demo_.get<CameraComponent>(my_entity_);

            auto& mc = registry_demo_.get<ModelComponent>(my_entity_);
            ...
            if (id_c.id == ball_id_) {
              auto ball_view =
                  registry_demo_.view<BallComponent, PhysicsComponent>();
    ...
    for (auto player : view_players) {
        EntityID id = registry_demo_.get<IDComponent>(player).id;
        auto& player_c = registry_demo_.get<PlayerComponent>(player);
        if (id == my_id_) {
    ...
    if (stun_timer_.Elapsed() >= my_stun_time_) {
        im_stunned_ = false;
        if (registry_demo_.has<ModelComponent>(my_entity_)) {
            registry_demo_.get<ModelComponent>(my_entity_).emission_strength =
                1.0f;
        }
        stun_timer_.Pause();
    }
...
void DemoState::UpdateNetwork() {
    ...
    // Record replay
    if (this->recording_) {
        this->engine_->GetReplayMachinePtr()->RecordFrame(this->registry_demo_);
    }
}

void DemoState::Cleanup() {
    engine_->GetSoundSystem().StopAmbientSound(registry_demo_);
...
void DemoState::ToggleInGameMenu() {
...
void DemoState::UpdateInGameMenu(bool show_menu) {
    // Set in_game buttons visibility
    auto view = registry_demo_.view<ButtonComponent, TransformComponent>();
    for (auto v : view) {
        auto& button_c = registry_demo_.get<ButtonComponent>(v);
        button_c.visible = show_menu;
    }
}

void DemoState::UpdateGameplayTimer() {
    int temp = 0;
    if (!overtime_has_started_) {
...
void DemoState::DrawNameOverPlayer() {
    auto player_view =
        registry_demo_.view<PlayerComponent, TransformComponent,
        ModelComponent, IDComponent>();

    auto& my_transform = registry_demo_.get<TransformComponent>(my_entity_);
...
void DemoState::DrawWallOutline() {
    if (my_primary_ability_id == (int)AbilityID::BUILD_WALL &&
        primary_cd_ <= 0.f ||
        engine_->GetSecondaryAbility() == AbilityID::BUILD_WALL) {
        auto& trans = registry_demo_.get<TransformComponent>(my_entity_);
        auto& camera = registry_demo_.get<CameraComponent>(my_entity_);
...
void DemoState::DrawFishingLines() {
    for (auto fisher : fishers_) {
        ...
        for (auto transform : view_transforms) {
            EntityID id = registry_demo_.get<IDComponent>(transform).id;
            glm::vec3 pos =
                registry_demo_.get<TransformComponent>(transform).position;
            ...
            if (id == fisher.owner_id) {
                ...
                player_pos = pos;
                player_forward =
                    registry_demo_.get<TransformComponent>(transform).Forward();
            }
...
void DemoState::UpdateSwitchGoalTimer() {
    // Countdown timer
...
#define LIGHT_EFFECT 2
    ...
    if (countdown_in_progress_) {
        ...
        if (dim < 0.5f) {
            auto& blue_light =
                registry_demo_.get<LightComponent>(blue_goal_light_);
            blue_light.color = glm::vec3(0.f);

            auto& red_light = registry_demo_.get<LightComponent>(red_goal_light_);
    ...
    if (countdown_in_progress_) {
        ...
        auto& blue_light = registry_demo_.get<LightComponent>(blue_goal_light_);
    ...
    if (switching_goals_) {
        ...
        if (temp_time - time <= 0) {
            ...
            auto& red_light =
            registry_demo_.get<LightComponent>(red_goal_light_); red_light.color
            = glm::vec3(1.f, 0.1f, 0.1f); red_light.radius = 30;*/
        }
    }
}

void DemoState::DrawTopScores() {
...
void DemoState::OnServerFrame() {
    auto& trans_c = registry_demo_.get<TransformComponent>(my_entity_);

    server_predicted_.position = new_transforms_[my_id_].first;
    server_predicted_.velocity =
        registry_demo_.get<PhysicsComponent>(my_entity_).velocity;
    server_predicted_.is_airborne =
        registry_demo_.get<PhysicsComponent>(my_entity_).is_airborne;
...
void DemoState::MoveBall(float dt) {
    auto view_ball =
        registry_demo_.view<BallComponent, TransformComponent,
...
void DemoState::BlackHoleMovement(float dt) {
    auto view = registry_demo_.view<TransformComponent, BlackHoleComponent>();
    auto& player_trans = registry_demo_.get<TransformComponent>(my_entity_);
...
void DemoState::Collision() {
    auto view_moveable =
        registry_demo_.view<TransformComponent, physics::OBB>();
    ...
    auto view_moveable1 =
        registry_demo_.view<TransformComponent, physics::Sphere>();
    ...
    auto& my_obb = registry_demo_.get<physics::OBB>(my_entity_);
    auto& my_phys_c = registry_demo_.get<PhysicsComponent>(my_entity_);
    auto& my_id = registry_demo_.get<IDComponent>(my_entity_);
    auto& arena_hitbox =
        registry_demo_.get<physics::MeshHitbox>(arena_entity_);
    auto& arena_hitbox2 =
        registry_demo_.get<FailSafeArenaComponent>(arena_entity_);
    ...
    auto view_walls = registry_demo_.view<physics::OBB>();
    for (auto wall : view_walls) {
        if (registry_demo_.has<PlayerComponent>(wall)) {
            continue;
        }
    ...
    // collision with ball
    auto view_ball = registry_demo_.view<BallComponent, physics::Sphere>();
    for (auto ball : view_ball) {
    ...
    auto view_player =
        registry_demo_.view<PlayerComponent, physics::OBB, IDComponent>();
    ...
    auto view_obb = registry_demo_.view<TransformComponent, physics::OBB>();
...
void DemoState::UpdateGravity() {
    auto view = registry_demo_.view<GravityComponent, TimerComponent>();
...
unsigned long DemoState::GetBestPlayer() {
...
EntityID DemoState::ClientIDToEntityID(long client_id) {
    return engine_->GetPlayerScores()[client_id].enttity_id;
}

void DemoState::DrawTarget() {
    if (my_target_ != -1 &&
        ...
        for (auto player : view_players) {
            auto& p_c = registry_demo_.get<PlayerComponent>(player);
            auto& t_c = registry_demo_.get<TransformComponent>(player);
            auto& id_c = registry_demo_.get<IDComponent>(player);
            ...
            if (id_c.id == my_id_) {
                ...
                if (registry_demo_.has<CameraComponent>(player)) {
                    auto& c_c = registry_demo_.get<CameraComponent>(player);
...
void DemoState::DrawJumbotronText() {
    glm::vec3 pos_base;
...
void DemoState::DrawMiniMap() {
    ...
    if (my_team_ == TEAM_RED) {
        ...
        auto view_ball =
            registry_demo_.view<TransformComponent, BallComponent>();
...
void DemoState::SetEntityTransform(EntityID player_id, glm::vec3 pos,
    glm::quat orientation) {
    transforms_[player_id] = std::make_pair(pos, orientation);
}

void DemoState::SetEntityPhysics(EntityID player_id, glm::vec3 vel,
    bool is_airborne) {
    physics_[player_id] = std::make_pair(vel, is_airborne);
}

void DemoState::SetCameraOrientation(glm::quat orientation) {
    registry_demo_.view<CameraComponent>().each(
        [&](auto entity, CameraComponent& cam_c) {
            cam_c.orientation = orientation;
        });
}

void DemoState::CreateInitialEntities() {
...
void DemoState::CreatePlayerEntities() {
    auto& sound_engine = engine_->GetSoundEngine();

    std::cout << "DEBUG: DemoState.cpp: Created " << player_ids_.size()
        << " players\n";

    for (auto entity_id : player_ids_) {
        auto entity = registry_demo_.create();

        glm::vec3 alter_scale =
            glm::vec3(5.509f - 5.714f * 2.f, -1.0785f, 4.505f - 5.701f * 1.5f);
        glm::vec3 character_scale = glm::vec3(0.0033f);

        registry_demo_.assign<IDComponent>(entity, entity_id);
        auto& pc = registry_demo_.assign<PlayerComponent>(entity);
        registry_demo_.assign<TransformComponent>(entity, glm::vec3(0.f),
            glm::quat(), character_scale);

        registry_demo_.assign<PhysicsComponent>(entity);
        registry_demo_.assign<SoundComponent>(entity,
            sound_engine.CreatePlayer());

        auto& model_c = registry_demo_.assign<ModelComponent>(entity);
        ...
        registry_demo_.assign<physics::OBB>(
            entity,
        ...
        if (entity_id == my_id_) {
            ...
            registry_demo_.assign<CameraComponent>(entity, camera_offset,
        ...
        auto& anim_c = registry_demo_.assign<AnimationComponent>(
            entity, glob::GetAnimationData(player_model));

        model_c.offset = glm::vec3(0.f, 0.9f, 0.f);

        auto& f = registry_demo_.assign<FollowBoneComponent>(entity);
        ...
        auto& part_c = registry_demo_.assign<ParticleComponent>(entity);
...
void DemoState::CreateArenaEntity() {
    auto arena = registry_demo_.create();
        ...
        for (int i = 0; i < 4; i++) {
            {
                ...
                registry_demo_.assign<TransformComponent>(entity, pos, glm::vec3(),
                    glm::vec3(1, 250, 10));
                registry_demo_.assign<SkyLightComponent>(entity);
            }
            {
                auto entity = registry_demo_.create();
                auto& model_c = registry_demo_.assign<ModelComponent>(entity);
...
void DemoState::CreateAudienceEntities() {
    glm::vec3 zero_vec = glm::vec3(0.0f);

    glob::ModelHandle model_audience = glob::GetModel(kModelPathAudience);

    for (int i = 0; i < 4; i++) {
        auto audience = registry_demo_.create();

        auto& audience_mc = registry_demo_.assign<ModelComponent>(audience);
        audience_mc.handles.push_back(model_audience);

        registry_demo_.assign<TransformComponent>(
            audience, zero_vec, glm::vec3(0.f, (pi / 2.f) * i, 0.f),
            glm::vec3(0.02f, 0.02f, 0.02f));

        auto& animation_c = registry_demo_.assign<AnimationComponent>(
...
void DemoState::CreateMapEntity() {
    auto arena = registry_demo_.create();
    glm::vec3 zero_vec = glm::vec3(0.0f);
    glm::vec3 arena_scale = glm::vec3(1.0f) * arena_scale_;
    ...
    registry_demo_.assign<FailSafeArenaComponent>(arena, a);
...
void DemoState::CreateBallEntities() {
    ...
    for (auto& [id, is_real] : init_balls_) {
        ...
        auto ball = registry_demo_.create();
        ...
        auto& model_c = registry_demo_.assign<ModelComponent>(ball);
        model_c.handles.push_back(model_ball_sphere_p);
        model_c.handles.push_back(model_ball_projectors_p);

        registry_demo_.assign<TransformComponent>(ball, zero_vec, zero_vec,
                                                      glm::vec3(0.95f));
        registry_demo_.assign<PhysicsComponent>(ball);
        auto& ball_c = registry_demo_.assign<BallComponent>(ball);

        registry_demo_.assign<IDComponent>(ball, ball_obj.first);
        registry_demo_.assign<SoundComponent>(ball,
                                                  sound_engine.CreatePlayer());
        registry_demo_.assign<physics::Sphere>(ball, glm::vec3(0.0f), 1.0f);
        registry_demo_.assign<TargetComponent>(ball);
        AddLightToBall(registry_demo_, ball);

        registry_demo_.assign<TrailComponent>(ball);
        */
    }
}

void DemoState::CreateSpotlights() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        auto entity = registry_demo_.create();
        ModelComponent& model_c = registry_demo_.assign<ModelComponent>(entity);
        glob::ModelHandle m_hndl = glob::GetModel(kModelPathSpotlight);
        // model_c.handles.push_back(m_hndl);
        TransformComponent& trans_c =
            registry_demo_.assign<TransformComponent>(entity);
...
void DemoState::CreateJumbotron() {
    glm::vec3 pos_base;
    pos_base.x = GlobalSettings::Access()->ValueOf("JUMBOTRON_POSITION_BASEX");
    ...
    float zrot = 1.f;
    for (int i = 0; i < 4; i++) {
        auto jumbo = registry_demo_.create();
        ModelComponent& model_c = registry_demo_.assign<ModelComponent>(jumbo);
        ...
        TransformComponent& trans_c =
            registry_demo_.assign<TransformComponent>(jumbo);
        trans_c.position =
            glm::vec3(xrot * pos_base.x, pos_base.y, zrot * pos_base.z);
...
void DemoState::ParticleComponentDestroyed(entt::entity e,
...
void DemoState::SoundComponentDestroyed(entt::entity e,
    entt::registry& registry) {
    auto& sc = registry.get<SoundComponent>(e);
    engine_->GetSoundEngine().DestroyPlayer(sc.sound_player);
}

void DemoState::CreateNewBallEntity(bool fake, EntityID id) {
    auto& sound_engine = engine_->GetSoundEngine();
    ...
    registry_demo_.assign<TransformComponent>(ball, zero_vec, zero_vec,
        glm::vec3(0.95f));
    registry_demo_.assign<PhysicsComponent>(ball);
    registry_demo_.assign<BallComponent>(ball);
    registry_demo_.assign<IDComponent>(ball, id);
    registry_demo_.assign<SoundComponent>(ball, sound_engine.CreatePlayer());
    registry_demo_.assign<physics::Sphere>(ball, zero_vec, 0.95f);

    registry_demo_.assign<TrailComponent>(ball);
}

void DemoState::CreateInGameMenu() {
    font_test_ = glob::GetFont("assets/fonts/fonts/ariblk.ttf");
    ...
    // CONTINUE BUTTON -- change registry to registry_demo_
    ButtonComponent* in_game_buttons_ = GenerateButtonEntity(
        registry_demo_, "CONTINUE", in_game_menu_pos + glm::vec2(0, 0),
        font_test_, false);
    in_game_buttons_->button_func = [&]() { ToggleInGameMenu(); };
    // SETTINGS BUTTON -- change registry to registry_settings_
    in_game_buttons_ = GenerateButtonEntity(registry_demo_, "SETTINGS",
    ...
    in_game_buttons_ = GenerateButtonEntity(registry_demo_, "MAIN MENU",
    ...
    in_game_buttons_ = GenerateButtonEntity(registry_demo_, "EXIT",
...
void DemoState::TestCreateLights() {
    // Create lights
    blue_goal_light_ = registry_demo_.create();
    registry_demo_.assign<LightComponent>(
        blue_goal_light_, glm::vec3(0.1f, 0.1f, 1.0f), 30.f, 0.0f);
    registry_demo_.assign<TransformComponent>(
        blue_goal_light_, glm::vec3(45.f, -8.f, 0.f), glm::vec3(0.f, 0.f, 1.f),
        glm::vec3(1.f));

    red_goal_light_ = registry_demo_.create();
    registry_demo_.assign<LightComponent>(
    ...
    auto light = registry_demo_.create();
    registry_demo_.assign<LightComponent>(light, glm::vec3(0.4f, 0.4f, 0.4f),
        90.f, 0.2f);
    registry_demo_.assign<TransformComponent>(
        light, glm::vec3(0, 4.f, 0.f), glm::vec3(0.f, 0.f, 1.f), glm::vec3(1.f));
}

void DemoState::AddPlayer() {
    player_ids_.clear();
    auto& sound_engine = engine_->GetSoundEngine();
    for (auto entity_id : player_ids_) {
        auto entity = registry_demo_.create();
        ...
        registry_demo_.assign<AnimationComponent>(
            entity, glob::GetAnimationData(player_model));
        registry_demo_.assign<PhysicsComponent>(entity);
        registry_demo_.assign<SoundComponent>(entity,
            sound_engine.CreatePlayer());
...
void DemoState::CreateWall(EntityID id, glm::vec3 position, glm::quat rotation,
    unsigned int team) {
    auto wall = registry_demo_.create();
    auto& sound_engine = engine_->GetSoundEngine();
    registry_demo_.assign<SoundComponent>(wall, sound_engine.CreatePlayer());
    registry_demo_.assign<IDComponent>(wall, id);
    registry_demo_.assign<TransformComponent>(wall, position, rotation,
        glm::vec3(1.f));
    auto& obb = registry_demo_.assign<physics::OBB>(wall);
    obb.extents[0] = 0.5f;
    obb.extents[1] = 6.2f;
    obb.extents[2] = 5.5f;
    ...
    auto& model_c = registry_demo_.assign<ModelComponent>(wall, hs);
    registry_demo_.assign<WallComponent>(wall);
    if (team == TEAM_BLUE) {
        model_c.diffuse_index = 1;
    }
    else {
...
void DemoState::CreatePickUp(EntityID id, glm::vec3 position) {
    auto pick_up = registry_demo_.create();

    registry_demo_.assign<IDComponent>(pick_up, id);

    glob::ModelHandle model_pick_up = glob::GetModel(kModelPathPickup);
    auto& model_c = registry_demo_.assign<ModelComponent>(pick_up);
    model_c.handles.push_back(model_pick_up);

    registry_demo_.assign<TransformComponent>(
        pick_up, position, glm::vec3(0.0f, 0.0f, 0.f), glm::vec3(0.4f));
    registry_demo_.assign<PickUpComponent>(pick_up);
    auto& snd = registry_demo_.assign<SoundComponent>(pick_up);
...
void DemoState::CreateCannonBall(EntityID id, glm::vec3 pos, glm::quat ori,
    unsigned int creator_team) {
    auto cannonball = registry_demo_.create();
    ...
    auto& model_c = registry_demo_.assign<ModelComponent>(cannonball);
    model_c.handles.push_back(model_shot);

    registry_demo_.assign<TransformComponent>(cannonball, pos, ori,
        glm::vec3(0.6f));
    registry_demo_.assign<ProjectileComponent>(cannonball,
        ProjectileID::CANNON_BALL);
    registry_demo_.assign<TrailComponent>(cannonball, 0.05f, color_vec);

    registry_demo_.assign<IDComponent>(cannonball, id);
}

void DemoState::CreateTeleportProjectile(EntityID id, glm::vec3 pos,
    glm::quat ori) {
    auto teleport_projectile = registry_demo_.create();
    glm::vec3 zero_vec = glm::vec3(0.0f);

    registry_demo_.assign<TrailComponent>(teleport_projectile, 0.5f,
        glm::vec4(1, 1, 1, 1));
    registry_demo_.assign<TransformComponent>(teleport_projectile, pos, ori,
        glm::vec3(0.3f));
    registry_demo_.assign<ProjectileComponent>(
        teleport_projectile, ProjectileID::TELEPORT_PROJECTILE);
    registry_demo_.assign<IDComponent>(teleport_projectile, id);
}

void DemoState::CreateForcePushObject(EntityID id, glm::vec3 pos,
    glm::quat ori) {
    auto& sound_engine = engine_->GetSoundEngine();

    auto force_object = registry_demo_.create();
    glm::vec3 zero_vec = glm::vec3(0.0f);
    glob::ModelHandle model_ball = glob::GetModel(kModelPathBall);
    //"assets/Ball/force_push_ball.fbx"	TODO: Swap path to this one
    auto& model_c = registry_demo_.assign<ModelComponent>(force_object);
    model_c.handles.push_back(model_ball);

    registry_demo_.assign<TransformComponent>(force_object, pos, ori,
        glm::vec3(0.5f));
    registry_demo_.assign<IDComponent>(force_object, id);
    registry_demo_.assign<SoundComponent>(force_object,
        sound_engine.CreatePlayer());
    registry_demo_.assign<ProjectileComponent>(
        force_object, ProjectileID::FORCE_PUSH_OBJECT);
    registry_demo_.assign<TrailComponent>(force_object, 1.f,
        glm::vec4(0.4, 0.4, 1, 1));
}

void DemoState::CreateMissileObject(EntityID id, glm::vec3 pos, glm::quat ori) {
    auto& sound_engine = engine_->GetSoundEngine();

    auto missile_object = registry_demo_.create();
    glm::vec3 zero_vec = glm::vec3(0.0f);
    glob::ModelHandle model_missile = glob::GetModel(kModelPathRocket);
    auto& model_c = registry_demo_.assign<ModelComponent>(missile_object);
    model_c.handles.push_back(model_missile);
    registry_demo_.assign<TransformComponent>(missile_object, pos, ori,
        glm::vec3(0.5f));
    registry_demo_.assign<IDComponent>(missile_object, id);
    registry_demo_.assign<SoundComponent>(missile_object,
        sound_engine.CreatePlayer());
    registry_demo_.assign<ProjectileComponent>(missile_object,
        ProjectileID::MISSILE_OBJECT);
    registry_demo_.assign<TrailComponent>(missile_object, 0.2f,
        glm::vec4(1.0f, 0.6f, 0.2f, 1.0f));
}

void DemoState::CreateFishermanAndHook(EntityID id, glm::vec3 pos,
    glm::quat ori, EntityID owner_id) {
    auto& sound_engine = engine_->GetSoundEngine();
    glob::ModelHandle hook_model = glob::GetModel(kModelPathHook);
    auto hook_object = registry_demo_.create();
    glm::vec3 zero_vec = glm::vec3(0.0f);
    auto& model_c = registry_demo_.assign<ModelComponent>(hook_object);
    model_c.handles.push_back(hook_model);
    registry_demo_.assign<TransformComponent>(hook_object, pos, ori,
        glm::vec3(0.3f));
    registry_demo_.assign<IDComponent>(hook_object, id);
    registry_demo_.assign<SoundComponent>(hook_object,
        sound_engine.CreatePlayer());
    registry_demo_.assign<ProjectileComponent>(hook_object,
        ProjectileID::FISHING_HOOK);

    registry_demo_.assign<HookComponent>(hook_object, owner_id);
...
void DemoState::CreateBlackHoleObject(EntityID id, glm::vec3 pos,
    glm::quat ori) {
    auto& sound_engine = engine_->GetSoundEngine();

    auto black_hole = registry_demo_.create();
    glm::vec3 zero_vec = glm::vec3(0.0f);
    glob::ModelHandle model_ball = glob::GetModel(kModelPathBall);
    //"assets/Ball/force_push_ball.fbx"	TODO: Swap path to this one
    auto& model_c = registry_demo_.assign<ModelComponent>(black_hole);
    model_c.handles.push_back(model_ball);

    registry_demo_.assign<TransformComponent>(black_hole, pos, ori,
        glm::vec3(0.3f));
    registry_demo_.assign<IDComponent>(black_hole, id);
    registry_demo_.assign<SoundComponent>(black_hole,
        sound_engine.CreatePlayer());
    registry_demo_.assign<PhysicsComponent>(black_hole);
    registry_demo_.assign<ProjectileComponent>(black_hole,
...
void DemoState::CreateMineObject(unsigned int owner_team, EntityID mine_id,
    glm::vec3 pos) {
    auto& sound_engine = engine_->GetSoundEngine();
    entt::entity mine_object = registry_demo_.create();

    if (owner_team == my_team_) {
        glob::ModelHandle model_mine =
            glob::GetModel(kModelPathMine);  // Switch to mine model
        ModelComponent& model_c =
            registry_demo_.assign<ModelComponent>(mine_object);
    ...
    registry_demo_.assign<IDComponent>(mine_object, mine_id);
    registry_demo_.assign<TransformComponent>(mine_object, pos);
    registry_demo_.assign<MineComponent>(mine_object, owner_team);
    registry_demo_.assign<SoundComponent>(mine_object,
        sound_engine.CreatePlayer());
    registry_demo_.assign<TimerComponent>(mine_object, 30.f);
}

void DemoState::DestroyEntity(EntityID id) {
    auto id_view = registry_demo_.view<IDComponent>();
    ...
    for (auto entity : id_view) {
        ...
        if (e_id.id == id) {
            if (registry_demo_.has<BallComponent>(entity)) {
                was_ball = true;

                if (registry_demo_.has<SoundComponent>(entity)) {
                    auto& sound_c = registry_demo_.get<SoundComponent>(entity);
                    auto ent = registry_demo_.create();
            ...
            if (registry_demo_.has<WallComponent>(entity)) {
                was_wall = true;
            }
            if (registry_demo_.has<TransformComponent>(entity)) {
                pos = registry_demo_.get<TransformComponent>(entity).position;
            }
            registry_demo_.destroy(entity);
            break;
        }
    }

    if (was_ball || was_wall) {
        auto e = registry_demo_.create();
        auto handle = glob::CreateParticleSystem();
        ...
        auto& p_c = registry_demo_.assign<ParticleComponent>(
            e, handles, offsets, directions);
        registry_demo_.assign<TimerComponent>(e, 5.f);
    }
}

void DemoState::SwitchGoals() {
    if (!goals_swapped_) {
        goals_swapped_ = true;
    }
    else {
    ...
    TransformComponent& blue_light_trans_c =
        registry_demo_.get<TransformComponent>(blue_goal_light_);
    TransformComponent& red_light_trans_c =
        registry_demo_.get<TransformComponent>(red_goal_light_);
    ...
    auto& map_trans =
        registry_demo_.get<TransformComponent>(map_visual_entity_);

    map_trans.rotation *= glm::quat(glm::vec3(0.f, glm::pi<float>(), 0.f));
}

void DemoState::SetPlayerLookDir(EntityID id, glm::vec3 look_dir) {
    player_look_dirs_[id] = look_dir;
}

void DemoState::SetPlayerMoveDir(EntityID id, glm::vec3 move_dir) {
    player_move_dirs_[id] = move_dir;
}

void DemoState::ReceiveGameEvent(const GameEvent& e) {
    ...
    case GameEvent::REMOVE_FISHING_HOOK: {
        ...
        auto view_hooks = registry_demo_.view<IDComponent>();
        for (auto hook : view_hooks) {
            auto& hook_id_c = registry_demo_.get<IDComponent>(hook);
            if (hook_id_c.id == id) {
                // registry_demo_.destroy(hook);
...
void DemoState::Reset() {
    auto destroy_view = registry_demo_.view<DestroyOnResetComponent>();
    registry_demo_.destroy(destroy_view.begin(), destroy_view.end());

    auto view_particle = registry_demo_.view<ParticleComponent>();
    switching_goals_ = false;
    for (auto& entity : view_particle) {
        if (registry_demo_.has<TimerComponent>(entity)) continue;
    ...
    pitch_ = 0.0f;
    if (registry_demo_.valid(my_entity_)) {
        auto& player_c = registry_demo_.get<PlayerComponent>(my_entity_);
        player_c.can_jump = false;
    }
...
void DemoState::EndGame() { game_has_ended_ = true; }

void DemoState::OverTime() {
    if (!overtime_has_started_) {
...
void DemoState::AddPitchYaw(float pitch, float yaw) {
    pitch_ += pitch;
    yaw_ += yaw;
    constexpr float pi = glm::pi<float>();
...
void DemoState::SetPitchYaw(float pitch, float yaw) {
    pitch_ = pitch;
    yaw_ = yaw;
    constexpr float pi = glm::pi<float>();
    pitch_ = glm::clamp(pitch_, -0.49f * pi, 0.49f * pi);
