...
namespace slob {
  ...
  public:
    void LoadFromFile(const std::string& path, FMOD::System* system) {
      FMOD_RESULT result = system->createSound(path.c_str(), FMOD_CREATECOMPRESSEDSAMPLE, nullptr, &sound_);
  ...
  void SoundPlayer::Set3DAttributes(glm::vec3 pos, glm::vec3 vel) {
    auto result =
      i_->channel_group->set3DAttributes((FMOD_VECTOR*)& pos, (FMOD_VECTOR*)& vel);
  }
